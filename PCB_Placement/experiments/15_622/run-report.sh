...
python generate_experiment_report.py --dir ${EXP_DIR}/work --hyperparameters ${EXP_DIR}/hyperparameters/hp_td3.json ${EXP_DIR}/hyperparameters/hp_sac.json ${EXP_DIR}/hyperparameters/hp_ddpg.json ${EXP_DIR}/hyperparameters/hp_dqn.json --report_config ${EXP_DIR}/report_config.json --output ${EXP_DIR}/experiment_report.pdf -y --tmp_dir ${EXP_DIR}/tmp
cd ${EXP_DIR}
...
if [ -e "$KICAD_PARSER" ] && [ -e "$SA_PCB" ] && [ -e "$PCB_ROUTER" ]; then
    ...
    DQN_EVAL_TESTING_DIR=${EXP_DIR}/work/eval_testing_set
