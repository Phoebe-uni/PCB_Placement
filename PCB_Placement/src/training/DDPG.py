...
class DDPG(object):
    def __init__(
        ...
        if verbose == 1:
            print(f"Model DDPG is configured to device {self.device}")
    ...
    def learn(self,
        ...
        for t in range(1,int(timesteps)+1):
            ...
            if t < start_timesteps:
                obs_vec = self.train_env.step(model=self.actor, random=True,  rl_model_type = 'DDPG')
            else:
                obs_vec = self.train_env.step(model=self.actor, random=False,  rl_model_type = 'DDPG')
