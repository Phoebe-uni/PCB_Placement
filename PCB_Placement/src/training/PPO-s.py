...
class Actor(nn.Module):
    ...
    def get_log_prob(self, state):
        ...
        action_dist = torch.distributions.Normal(action_mean, action_std)
        action = action_dist.sample()
        action_logprob = action_dist.log_prob(action.item())
        #print(f"logprob: {action_logprob},  {action_logprob.item()}")
        return action_logprob.detach()
        '''
...
class PPO(object):
    ...
    def train(self, replay_buffer):
        ...
        old_action_probs, old_dist_entropys = self.actor.get_log_prob1(old_state)
        advantages = rewards - old_action_probs.view(-1,1)
        advantages = advantages.view(-1)
        new_action_probs = self.actor.get_log_prob(state)
        if self.total_it%500 == 1:
            print(f"advantages shape: {advantages.shape}, rewards shape:{rewards.shape}")
            print(f"new_action_probs shape: {new_action_probs.shape}")
        ratios = torch.exp(new_action_probs - old_action_probs).view(-1)
        ...
        if self.total_it%500 == 1:
            ...
            print(f"old next state shape: {old_state.shape}, {next_state.shape}")
            print(f"advantages shape: {advantages.shape}, rewards shape:{rewards.shape}")
            print(f"action_probs shape: {old_action_probs.shape}, ratios shape: {ratios.shape}")
            print(f"loss: {loss}")
        ...
        '''
        #old_state_values = self.old_critic(state)
