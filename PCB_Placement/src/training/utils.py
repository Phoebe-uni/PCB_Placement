...
Transition = namedtuple(
    'Transition', ('state', 'action', 'next_state', 'reward', 'done', 'old_state'))
...
class ReplayMemory(object):
    ...
    def sample(self, batch_size):
        ...
        #print(f"batch.log_prob: {batch.log_prob}")
        old_state = torch.FloatTensor(np.concatenate(batch.old_state)).to(self.device)
        return state, action, next_state, reward, done, old_state
    ...
    def sample_from_latest(self, batch_size, latest):
        ...
        log_prob = torch.FloatTensor(np.concatenate(batch.log_prob)).to(self.device)
        return state, action, next_state, reward, done, log_prob
