    ...
    def train(self, i_ep):
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), self.batch_size, False):
                ...
                critic_losses.apppend(value_loss)
                if self.training_step % 1000 ==0:
        ...
        actor_loss = float(np.mean(np.array(actor_losses)))
        critic_loss = float(np.mean(np.array(critic_losses)))
    ...
    def learn(self,
        ...
        for t in range(1,int(timesteps)+1):
            ...
            for indiv_obs in obs_vec:
                ...
                trans = PPOTransition(indiv_obs[0], action, action_prob, indiv_obs[3], indiv_obs[2])
