...
class PPO(object):
    ...
    def train(self, replay_buffer):
        ...
        for i in range(self.K_epochs):
            ...
            old_log_probs, old_dist_entropys = self.actor.get_log_prob1(state)
            old_state_values = self.critic(state, action)

            log_probs, dist_entropys = self.actor_target.get_log_prob1(next_state)
            state_values = self.critic_target(next_state, next_action)

            cur_values = rewards + not_done * self.discount * state_values

            critic_loss = self.MseLoss(old_state_values, cur_values)
            ...
            if self.training_step % 200 == 0 and i == 0:
                ...
                print(f"cur_values shape: {cur_values.shape}, critic_loss: {critic_loss}, actor_loss shape:{actor_loss.shape} ")
            ...
            actor_losses.append(actor_loss.mean().detach().numpy())
