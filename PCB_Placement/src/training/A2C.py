...
class A2C(object):
    def __init__(
        ...
        if verbose == 1:
            print(f"Model A2C is configured to device {self.device}")
    ...
    #        print(f"value_loss: {value_loss}")
        ...
        actor_losses.append(policy_loss.detach().cpu().numpy())
        ...
        if self.total_it % 5000 == 0:
            print(f"A2C {self.total_it} critic_loss: {critic_loss}, actor_loss: {actor_loss}")
    ...
    def learn(self,
        ...
        for t in range(1,int(timesteps)):
            ...
            if t < start_timesteps:
                obs_vec = self.train_env.step(model=self.actor, random=True,  rl_model_type = 'A2C')
            else:
                obs_vec = self.train_env.step(model=self.actor, random=False,  rl_model_type = 'A2C')
            ...
            if self.done:
                ...
                if t < start_timesteps:
                    ...
                    self.trackr.append(actor_loss=actor_loss,
                           ...
                           episode_fps = episode_timesteps / (episode_finish_time - episode_start_time))
                if t%5000 == 0:
                    print(f"A2C {t}, actor_loss: {actor_loss}, critic_loss: {critic_loss}, ep_reward: {episode_reward}")
                    print(f"A2C {t}, epi_length: {episode_timesteps}, epi_fps: {episode_timesteps / (episode_finish_time - episode_start_time)}")
