...
class PPO(object):
    ...
    def train(self, replay_buffer):
        ...
        self.total_it += 1
        state, action, next_state, reward,  not_done, old_action_log_prob1 = replay_buffer.sample(self.batch_size)
        ...
        #action_prob = self.actor(state).gather(1, action) # new policy
        #print(f"next state: {next_state}")
        old_action_log_prob = self.actor.get_log_prob(state)
        action_prob = self.actor.get_log_prob(next_state)
        #print(f"old_action_log_prob : {old_action_log_prob}")
        #print(f"old_action_log_prob1 : {old_action_log_prob1}")
        ...
        actor_losses.append(action_loss.detach().numpy())
        ...
        critic_losses.append(value_loss.detach().numpy())
        if self.training_step % 1000 ==0:
    ...
    def learn(self,
        ...
        for t in range(1,int(timesteps)+1):
            ...
            for indiv_obs in obs_vec:
                ...
                transition = (indiv_obs[0], indiv_obs[3], indiv_obs[1], indiv_obs[2], indiv_obs[4], 1. - indiv_obs[5])
