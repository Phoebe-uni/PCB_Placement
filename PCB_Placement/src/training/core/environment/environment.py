    ...
    def step(self,
        ...
        for i in idxs:
            if (rl_model_type != 'PPO'):
                state, next_state, reward, action, done, old_state = self.agents[i].step(
                    model=model,
                    random=random,
            ...
            _old_state = list(old_state["los"]) + list(old_state["ol"]) + old_state["dom"] + old_state["euc_dist"] + old_state["position"] + old_state["ortientation"]
            _next_state_info = next_state["info"]
            observation_vec.append(
                [_state, _next_state, reward, action, done, _old_state, _next_state_info])
